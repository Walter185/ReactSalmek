import { useReducer, createContext, useEffect } from "react";us
...
function EstadoProveedor(props) {
    ...
    useEffect(() => {
        if (localStorage.getItem("IdToken")) {
            ...
            const clienteRef = doc(
                db, `${rolUsuario === "administrador" ? "Personales" : "Clientes"}`,
    ...
    const disminuirCantiad = (IdProducto, Unidades) => {
        dispatch({
    ...
    const totalAPagar = () =>
        state.productos?.reduce(
            (antes, actual) => parseFloat(actual.Precio) * actual.Unidades + antes, 0 );
